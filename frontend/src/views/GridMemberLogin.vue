        ...
        <div class="login-box">
            ...
            <el-form ref="formRef" :rules="data.rules" :model="data.form" style="margin-top: 20px;">
                <el-form-item prop="gmCode">
                    <el-input v-model="data.form.gmCode" placeholder="请输入登录编码" prefix-icon="User" />
                </el-form-item>
...
const login = () => {
    formRef.value.validate((valid) => {
        if (valid) {
            ...
            request.post('/gridMemberLogin', data.form).then((res) => {
                if (res.code === '200') {
                    ElMessage.success('登录成功');
                    localStorage.setItem('gridMember-user', JSON.stringify(res.data));
                    setTimeout(() => {
                        location.href = '/gridMember';
                    }, 500);
