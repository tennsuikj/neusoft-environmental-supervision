        ...
        <div style="display: flex;">
            ...
            <div style="width: 200px;border-right: 1px solid #ddd; min-height: calc(100vh - 60px)">
                <el-menu router :default-active="$router.currentRoute.value.path" :default-openeds="['1']"
                    ...
                    <el-menu-item index="/decisionMaker">
                        <el-icon>
                            <House />
                        </el-icon>
                        系统首页
                    ...
                    <el-menu-item index="/decisionMaker/visionData">
                        <el-icon>
                            <ChatDotRound />
                        </el-icon>
                        <span>可视化数据</span>
...
const data = reactive({
    user: JSON.parse(localStorage.getItem('decisionMaker-user') || '{}')
})

//退出登录
const logout = () => {
    localStorage.removeItem('decisionMaker-user');//清除登录信息
    location.href = '/decisionMakerLogin';
}

//更新用户信息,接收子组件传递过来的事件
const updateUser = () => {
    data.user = JSON.parse(localStorage.getItem('decisionMaker-user') || '{}')
}
</script>
