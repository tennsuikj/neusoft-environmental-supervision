        ...
        <div class="login-box">
            ...
            <el-form ref="formRef" :rules="data.rules" :model="data.form" style="margin-top: 20px;">
                <el-form-item prop="adminCode">
                    <el-input v-model="data.form.adminCode" placeholder="请输入登录编码" prefix-icon="User" />
                </el-form-item>
...
const login = () => {
    formRef.value.validate((valid) => {
        if (valid) {
            ...
            request.post('/adminsLogin', data.form).then((res) => {
                if (res.code === '200') {
                    ElMessage.success('登录成功');
                    localStorage.setItem('admins-user', JSON.stringify(res.data));
                    setTimeout(() => {
